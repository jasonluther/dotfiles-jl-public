...
## Deploying to a remote machine (snoopy, lucy, …)
...
Status as of 2026-07-12: deployed on **schroeder** — suspend/resume and
magic-packet delivery verified, but an actual wake-from-sleep has _failed_
once (packets reach the NIC and the OS side is fully armed; BIOS wakeup
settings are the prime suspect, check pending). Note `rtcwake` round trips
prove suspend/resume only — they wake by RTC timer and say nothing about
WoL. Pending on
**snoopy** (awake; schroeder's key not yet authorized — run the dotfiles
overlay update there or apply this from the Mac) and **lucy** (needs manual
power-on first; magic packets don't wake it yet). MAC inventory for `wake`
lives in dotfiles-jl `dot_config/wake/hosts`.
